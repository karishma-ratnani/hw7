firebase.auth().onAuthStateChanged(async function(user) {
  ...
  if (user) {
    //log user in collections
    db.collection('users').doc(user.uid).set({
      name: user.displayName,
      email: user.email
    })

    //update page with user watched data
  for (let i=0; i<movies.length; i++) {
    let movie = movies[i]
    let userId = user.uid
    let movieId = `${movie.id}`
    //let docRef = await db.collection('watched').doc(`${movie.id}`).get()
    let docRef = await db.collection('watched').where('userId','==',user.uid).where('movieId','==',`${movie.id}`).get()
    let watchedMovie = docRef.docs[0]
    ...
    //make opaque when clicked and log into collection with userid and movieid
